...
async def upload_csv_for_model_creation_or_update(
    ...
    try:
        # -------------------------
        # 1️⃣ Create training job (Supabase generates job_id)
        # -------------------------
        ...
        # -------------------------
        set_progress(job_id, "validating", "Validating CSV")
        df = validate_training_csv(tmp_path)

        set_progress(job_id, "feature_engineering", "Building features")
        df_cleaned = clean_data_for_training_model(df)

        save_cleaned_csv(df_cleaned, asset_type, job_id)

        # -------------------------
        # 4️⃣ Background training task
        # -------------------------
        async def training_task():
            try:
                set_progress(job_id, "training", "Training model")
                model = train_model(df_cleaned, asset_type)

                set_progress(job_id, "saving_model", "Saving model")
                ...
                set_progress(job_id, "registering_model", "Registering model")
                ...
                set_progress(job_id, "completed", "Training completed")
            except Exception as e:
                set_progress(job_id, "failed", str(e))

        asyncio.create_task(training_task())

        # -------------------------
        # 5️⃣ Return job_id to frontend
        # -------------------------
        return {
